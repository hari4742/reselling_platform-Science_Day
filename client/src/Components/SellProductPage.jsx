...
const SellProductPage = () => {
  ...
  const uploadImages = async () => {
    ...
    if (files.length === 0) {
      ...
      const res = await backend.post(
        `/user/${user_id}/prod/${prod_id}/upload/product/images`,
        formdata,
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      formdata.set(
        "display_img",
        document.getElementById("display_img").files[0]
      );
      const res = await backend.post("/product/add", formdata, {
        headers: { "Content-Type": "multipart/form-data" },
      });
  ...
  return (
    <div className="sell-product-page">
      ...
      <div className="sell-product-page-btns">
        {!prod_id ? <p onClick={handleSubmit}>Upload More Images</p> : null}
        {prod_id ? (
